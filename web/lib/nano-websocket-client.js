(function() {
  ...
  var connect = function(params, url, cb) {
    ...
    var onopen = function(event) {
      nano.emit('net-open', event, !!reconnect);
      reset();
      var obj = Package.encode(Package.TYPE_HANDSHAKE, Protocol.strencode(JSON.stringify(handshakeBuffer)));
      send(obj);
    };
    var onmessage = function(event) {
      nano.emit('net-message', event);
    ...
    var onerror = function(event) {
      nano.emit('net-error', event);
      console.error('socket error: ', event);
    };
    var onclose = function(event) {
      nano.emit('net-close',event);
  ...
  nano.getSocket = function() {
    return socket;
  };

  nano.getSocketState = function() {
  ...
  var handshake = function(data) {
    ...
    if(data.code === RES_OLD_CLIENT) {
      nano.emit('heartbeat-error', 'client version not fullfill');
      return;
    }

    if(data.code !== RES_OK) {
      nano.emit('heartbeat-error', 'handshake fail');
      return;
    }
